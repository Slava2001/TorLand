...
| **div&nbsp;[reg1]&nbsp;[reg2]** | операция целочисленного деления, делит значение `reg1` на значение `reg2` и сохраняет значение в `reg1` |
| **divv&nbsp;[reg]&nbsp;[val]** | операция целочисленного деления, делит значение `reg` на значение `val` и сохраняет значение в `reg`. При переполнении `reg`, устанавливает флаг `FO`, иначе сбрасывает его |
| **mod&nbsp;[reg1]&nbsp;[reg2]** | операция деления по модулю, вычисляет остаток от деления значения `reg1` на значение `reg2` и сохраняет его в `reg1`  |
| **modv&nbsp;[reg]&nbsp;[val]** | операция деления по модулю, вычисляет остаток от деления значения `reg` на значение `val` и сохраняет его в `reg`. При переполнении `reg`, устанавливает флаг `FO`, иначе сбрасывает его |
